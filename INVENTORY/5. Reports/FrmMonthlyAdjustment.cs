...
namespace PMIS
{
    public partial class FrmMonthlyAdjustment : Form
    {
        public FrmMonthlyAdjustment()
        {
            InitializeComponent();
        }
        ...
        {
            MyForm.Combo.Fill(this.CboMonth, "SELECT * FROM vw_stocks_dtl_ym", "YM", "YM_ID");
            HasLoad = false;
            this.Fill();
        }
        ...
        void Fill()
        {
            ...
            {
                String s =  CboMonth.SelectedValue.ToString();
                String[] g = Regex.Split(s, "-");
                y = g[0];
                m = g[1];
            }
            ...
            String sql = "SELECT *,((Stock_Begin+Stock_Purchase)-Stock_Out) AS Stock_End FROM ( " +
                            "SELECT I.ItemNo,I.Stock_No,I.ItemUnit,I.ItemName,I.ItemDescription, " +
                                ...
                                "ABS(ISNULL((SELECT SUM(Quantity) FROM vw_stocks_dtl WHERE Item_No=I.ItemNo AND YEAR(TransDate)=" + y + " AND MONTH(TransDate)<" + m + " GROUP BY YEAR(TransDate), MONTH(TransDate), Item_No),0)) AS Stock_Begin, " +
                                "ABS(ISNULL((SELECT SUM(Quantity) FROM vw_stocks_dtl WHERE TransType='DELIVERY' AND Item_No=I.ItemNo AND Item_No=I.ItemNo AND YEAR(TransDate)=" + y + "AND MONTH(TransDate)=" + m + " GROUP BY YEAR(TransDate), MONTH(TransDate), Item_No),0)) AS Stock_Purchase, " +
                                "ABS(ISNULL((SELECT SUM(Quantity) FROM vw_stocks_dtl WHERE TransType='ISSUED' AND Item_No=I.ItemNo AND Item_No=I.ItemNo AND YEAR(TransDate)=" + y + " AND MONTH(TransDate)=" + m + " GROUP BY YEAR(TransDate), MONTH(TransDate), Item_No),0)) AS Stock_Out " +
                            "FROM vw_item I " +
        ...
        {
            Export.ToExcel.AdjustmentMonthly(dt, y, m);
        }
